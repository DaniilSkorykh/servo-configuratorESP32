...
class ServoDevice:
    ...
    def stop(self, *, emergency: bool = False) -> None:
        ...
        При ``emergency`` дополнительно снимается момент удержания, а
        устройство запирается в состоянии аварийного останова: движение
        останется запрещённым до вызова :meth:`reset`.
        """
    ...
    def reset(self) -> None:
        ...
        Отдельная команда, а не разновидность ``stop``: снятие подтверждает,
        что причина устранена и приводить механизм в движение безопасно.
        """
        self._client.request(Command.RESET)

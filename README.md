...
- **Обнаружение молчания.** Порт может остаться открытым, а данные
  прекратиться; такой случай выявляется по перерыву в потоке телеметрии.
...
**Чем аварийный останов отличается от обычного.** Обычный `stop` оставляет
момент удержания: привод стоит и сопротивляется внешнему усилию. Аварийный
дополнительно снимает момент — вал становится свободным, его можно повернуть
рукой, чтобы вывести механизм из опасного положения.

Аварийный останов не запирает устройство: оно сразу переходит в `idle`.
Чтобы продолжить работу, достаточно любой команды движения — момент
возвращается автоматически. Отдельного сброса не требуется.
